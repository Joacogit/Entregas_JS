...
for (const elemento of evento_boton_agregar) {
    elemento.addEventListener("click", (event) => {                         // ⇒ ADDEVENTLISTENER solo se puede aplicar a un solo ELem. del DOM a la vez
        ...
        tr.innerHTML =`
            ...
            <button id="delete" class="card-link btn btn-primary"><strong> X </strong></button>
            <hr>
            `
        ...
        dom_carrito.append(tr)          // ⇒ con append lo agrega al final de la tabla del DOM
         ...
         /*const delete_producto = document.getElementsById("delete")
          for (const elemento of delete_producto) {
...
const cargarOpdeCuotas = async () => {
    ...
    //al select del html cargar respectivas opciones 
    const select = document.getElementById("opcdecuotas");
    select.innerHTML = data.map((opcdecuotas) => `<option value= "${opcdecuotas.id}">${opcdecuotas.numero}</option>`);       //data son las op de cuotas - map dev un array 
}                           //map: dev un array 
cargarOpdeCuotas();
...
//const logo_footer = document.getElementsByClassName('logo_footer').addEventListener('mouseover', () => {
    Toastify({
